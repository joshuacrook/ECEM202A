...
#define FIFO_SIZE     32*ELEMENT_SIZE
//#define SAMPLES       32
#define THRESHOLD     50

bool APDS9960::updateReadings(uint8_t *buf, uint16_t buf_size) {
  // gestureAvailable
  if (!_gestureEnabled) enableGesture();
  if (_intPin > -1) {
    if (digitalRead(_intPin) != LOW) {
  ...
  if (available > 0) {
    uint16_t bytes_read = readGFIFO_U(fifo_data, available * 4); // reads up to FIFO_SIZE
    
    if (fifo_data[0] + fifo_data[1] + fifo_data[2] + fifo_data[3] > THRESHOLD) {
      //Serial.println("Begin capture");
      memcpy(buf, fifo_data, bytes_read);
      while (bytes_read < buf_size) {
        available = gestureFIFOAvailable();
        if (available > 0) {
          bytes_read += readGFIFO_U(buf + bytes_read, available * ELEMENT_SIZE);
        }
      }
